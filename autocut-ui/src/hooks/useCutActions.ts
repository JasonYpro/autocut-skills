...
import type { Word, ProjectState, PendingTextChange, SubtitleStylePreset, UserEditsPayload, ExportOptionsState } from '../types';
...
export function useCutActions({
  ...
  const handleExecuteCut = async () => {
    ...
    try {
      const data = await executeCut(currentProjectId, payload.deletes, burnSubtitle, subtitleStyle, userEdits || undefined, exportOptions);
      clearInterval(timer);
      ...
      if (data.success) {
        ...
        const speedMsg = exportOptions.speed !== 1 ? `\n${t.speed}: ${exportOptions.speed}x` : '';
        const denoiseMsg = exportOptions.denoise ? `\n${t.denoise}: ✓` : '';
        const voiceMsg = exportOptions.enhanceVoice ? `\n${t.enhanceVoice}: ✓` : '';
        showMessageDialog(
          'success',
          t.cutDone,
          `✅ ${t.cutDone} (${totalTime}s)\n\n📁 ${t.output}: ${data.output}${subtitleMsg}${speedMsg}${denoiseMsg}${voiceMsg}\n\n${t.originalDuration}: ${formatDuration(data.originalDuration || 0, t)}\n${t.newDuration}: ${formatDuration(data.newDuration || 0, t)}\n${t.deleted}: ${formatDuration(data.deletedDuration || 0, t)} (${data.savedPercent}%)`
        );
      } else {
