...
export function useVideoPlayerState({
  ...
  const registerVideoElement = useCallback(
    ...
    [videoReady, currentProjectId, loadVideoSrc, videoRef]
  );
  ...
  useEffect(() => {
    ...
    const syncVideoState = () => {
      ...
      if (previewSpeed && previewSpeed !== 1) {
  ...
  }, [currentProjectId, getVideoElement, videoRef, previewSpeed]);

  useEffect(() => {
    const video = getVideoElement(currentProjectId);
    if (!video) return;
    if (previewSpeed && previewSpeed > 0) {
      video.playbackRate = previewSpeed;
    }
  }, [currentProjectId, previewSpeed, getVideoElement]);
